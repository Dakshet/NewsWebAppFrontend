...
const NewsState = (props) => {
    ...
    //Add Magazine
    const addMagazine = async (formData) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (json.news) {
                    window.open(`${host}/files/../uploads/${json.news.body}`, "_blank", "noreferrer");
    ...
    //Delete Magazine
    const deleteMagazine = async (id, coverImageURL) => {

        try {

            const response = await fetch(`${host}/news/deletemagazine?id=${id}&coverImage=${coverImageURL}}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    "auth_token": localStorage.getItem("inews")
    ...
    return (<NewsContext.Provider value={{ news, fetchNews, pageNews, fetchPageSpecificNews, getNewsUsingId, specificNews, addNews, deleteNews, editNews, commentNews, fetchComment, addComment, searchNewsResult, setSearchNewsResult, fetchSearchNews, loginUserInfo, addMagazine, deleteMagazine }}>
        {props.children}
