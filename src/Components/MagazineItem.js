...
const MagazineItem = ({ mNews, showAlert }) => {
    ...
    const handleClick = (body) => {
        window.open(`${host}/files/../uploads/${body}`, "_blank", "noreferrer");
    }


    const handleDelete = (id, coverImageURL) => {
        showAlert("Deleted News Successfully!", "success");
        deleteMagazine(id, coverImageURL);
    }
    ...
    return (
        <div className="magazineParticularBoxOuter">
            ...
            <i className="ri-close-line closeIcon" onClick={() => handleDelete(mNews._id, mNews.coverImageURL)}></i>
        </div>
